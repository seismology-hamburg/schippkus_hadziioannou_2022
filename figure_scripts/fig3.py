...
def plot_beampowers_on_map(
    ...
    if settings["geometry_type"] == "geographic":
        ...
        if ax_id == 0:
            ...
            figlbl = "b)"
            labeltext = "Correction for spreading \n(surface wave)"
            ax.set_xticklabels([])
            ax.set_yticklabels([])
        elif ax_id == 2:
            figlbl = "c)"
            labeltext = "Time-domain normalisation of GF"
            if LHS:
                ax.set_yticklabels([-50, 0, 50], fontsize=6)
                ax.set_ylabel("Distance [km]", fontsize=6)
                ax.set_xticklabels([-50, 0, 50], fontsize=6)
        ...
        if ax_id in [1, 2]:
            ...
            maxima = beampowers == maximum_filter(beampowers, size=100, mode="constant")
            ...
            if ax_id == 2:
                threshold = 0.9
            localmaxlons[maxima_values <= threshold] = np.nan
            localmaxlats[maxima_values <= threshold] = np.nan
            ax.scatter(
                localmaxlons,
                localmaxlats,
                facecolors="#E2001A",
    ...
    if ax_id == 100:
        # 65,-66.33
        # -20,-26.33
        ax.set_xlim(-20 - 2.5, -20 + 2.5)
        ax.set_ylim(-26.33 - 2.5, -26.33 + 2.5)
        ax.set_xticks([])
        ax.set_yticks([])
        # ax.set_xticks([64, 66])
...
with open(
    "/data/cen/u254/sven/global_mfp/projects/173_redo_ampl_none/1636098974_settings.yml",
    "r",
) as stream:
...
fig.subplots_adjust(left=0.1, right=0.875, top=0.9, bottom=0.1)
outer_grid = fig.add_gridspec(2, 2, wspace=0.1, hspace=0.1)
...
ax_notreatment = fig.add_subplot(outer_grid[0, 0])
bp_fn = f"/data/cen/u254/sven/global_mfp/projects/173_redo_ampl_none/out/out_1548982800.0_3880_[0.13, 0.15]_0_0_False.npy"
...
ax_notreatment.plot(
    ...
    [-26.33 - 2.5, -26.33 + 2.5, -26.33 + 2.5, -26.33 - 2.5, -26.33 - 2.5],
    c="k",
    lw=1,
    zorder=100,
)

# zoom onto highest peak

# ax_notreatment_zoom = fig.add_axes([0.135, 0.66, 0.2, 0.2])
ax_notreatment_zoom = fig.add_axes([0.115, 0.66, 0.15, 0.15])

# ax_notreatment_zoom.spines["bottom"].set_color("#E2001A")
# ax_notreatment_zoom.spines["top"].set_color("#E2001A")
# ax_notreatment_zoom.spines["right"].set_color("#E2001A")
# ax_notreatment_zoom.spines["left"].set_color("#E2001A")
...
ax_timedomain = fig.add_subplot(outer_grid[1, 0])
bp_fn = f"/data/cen/u254/sven/global_mfp/projects/173_redo_ampl_timedomain/out/out_1548982800.0_3880_[0.13, 0.15]_0_0_False.npy"
# bp_fn = f"/data/cen/u254/sven/global_mfp/projects/160_ampl_treatment_time_domain/out/out_1548982800.0_3880_[0.13, 0.15]_0_0.npy"
beampowers = np.load(bp_fn)
beampowers /= np.max(np.abs(beampowers))
(
    grid_lon_coords,
...
ax_phasecorr = fig.add_subplot(outer_grid[0, 1])
#  bp_fn = f"/data/cen/u254/sven/global_mfp/projects/160_ampl_treatment_phase_corr/out/out_1548982800.0_3880_[0.13, 0.15]_0_0.npy"
bp_fn = f"/data/cen/u254/sven/global_mfp/projects/173_redo_ampl_spreading/out/out_1548982800.0_3880_[0.13, 0.15]_0_0_False.npy"
# bp_fn = f"/data/cen/u254/sven/global_mfp/projects/170_redo_synths_fig2_ampl_surface_wave_correction/out/out_1548982800.0_3880_[0.13, 0.15]_0_0_False.npy"
beampowers = np.load(bp_fn)
beampowers /= np.max(np.abs(beampowers))
(
    grid_lon_coords,
...
ax_whitening = fig.add_subplot(outer_grid[1, 1])
# bp_fn = f"/data/cen/u254/sven/global_mfp/projects/159_synth_test_GF/out/out_1548982800.0_3880_[0.13, 0.15]_0_0.npy"
bp_fn = f"/data/cen/u254/sven/global_mfp/projects/173_redo_ampl_whitening/out/out_1548982800.0_3880_[0.13, 0.15]_0_0_False.npy"
beampowers = np.load(bp_fn)
beampowers /= np.max(np.abs(beampowers))
(
    grid_lon_coords,
...
cb_ax = fig.add_axes([x0 + w + 0.05 * w, y0 + 0.5 * h, 0.05 * w, h])
cbar = plt.colorbar(pcm, cax=cb_ax)  # , extend="min")
cbar.set_ticks([-1, 0, 1])
cbar.set_label(label="Normalised Beampower", fontsize=6)
cbar.ax.tick_params(labelsize=6)

fig.savefig("/data/cen/u254/sven/global_mfp/figure_scripts/png/fig3.png", dpi=300)
